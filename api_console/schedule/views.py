...
class ScheduleViewSet(viewsets.ViewSet):
    ...
    def create(self, request):
        ...
        create_translation(translations=request.data['translations'],
                           model=Schedule, object_id=schedule.id)
        result_schedule = Schedule.objects.translate(
            request.user.language).get(id=schedule.id)
    ...
    def partial_update(self, request, pk=None):
        ...
        update_translation(translations=request.data['translations'],
                           model=Schedule, object_id=pk)
        result_schedule = Schedule.objects.translate(
            request.user.language).get(id=pk)
        return Response(ScheduleListSerializer(result_schedule).data)
